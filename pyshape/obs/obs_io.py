#Last modified by @recannon 10/01/2026

from dataclasses import dataclass
from typing import Union, Type
from ..utils import time_shape2astropy, time_astropy2shape
from ..cli_config import logger, error_exit
from astropy.time import Time

#These classes have less utility than the modFile class.
#Weights can be changed with the relevant function but editing dataclass objects
#will not be reflected when writing back out. (unless its weights)


class obsFile:
    
    def __init__(self, datasets, file_lines=None):
        self.datasets = datasets
        self.file_lines = file_lines

    def _extract_datasets(self):
        #Number of sets
        lines = self.file_lines
        idx = self._find_line('{number of sets}', lines)
        no_sets = int(lines[idx].split()[0])        
        logger.debug(f'Found {no_sets} sets')
        
        datasets = []
        for i in range(no_sets):
            
            set_start = self._find_line(f'SET {i}',lines)
            if i != no_sets - 1:
                set_end = self._find_line(f'SET {i+1}',lines)
            else:
                set_end = None
            
            set_lines = lines[set_start:set_end]
            datasets.append(ObsSet.from_lines(set_lines))
        
        self.datasets = datasets
    
    @staticmethod
    def _find_line(name: str, lines: list[str]) -> int:
        idx = next((i for i, line in enumerate(lines) if name in line), None)
        if idx is None:
            error_exit(f'Error: No {name} found in this file')
        return idx
    
    #===Factory methods===
    @classmethod
    def from_lines(cls, lines):
        """Parse a mod file from a list of raw lines."""
        obj = cls([], file_lines=lines)
        obj._extract_datasets()
        return obj

    @classmethod
    def from_file(cls, fname):
        """Parse a mod file directly from disk."""
        with open(fname) as f:
            lines = f.readlines()
        return cls.from_lines(lines)
    
    def write(self, fname=None):
        
        #Shape meta info
        output_lines = [self.file_lines[0],'\n']
        
        #Number of sets
        output_lines.append(f'{len(self.datasets): >14} {{number of sets}}')
        output_lines.append('\n\n\n')

        for ds in self.datasets:
            output_lines.extend(ds.set_lines)

        if fname:
            with open(fname, 'w') as f:
                f.writelines(output_lines)
            logger.debug(f'Written to {fname}')
        else:
            return ''.join(output_lines)
        
        

#==================
#   DATACLASSES
#==================

@dataclass
class ObsSet():
    set_lines: list[str]
    set_no: int
    set_type: str
    scattering_law: int
    
    @classmethod
    def from_lines(cls, set_lines: list[str]) -> "ObsSet":
        set_no = int(set_lines[0].split()[-1].rstrip('}'))
        idx = cls._find_line('{set type}', set_lines)
        set_type = set_lines[idx].split()[0]
        idx = cls._find_line('scattering law for this set}', set_lines)
        scattering_law = int(set_lines[idx].split()[0])

        #Return specific subclass for datatype
        subclass = cls._get_subclass(set_type)
        return subclass(set_lines=set_lines, set_no=set_no, 
                        set_type=set_type, scattering_law=scattering_law)

    def set_weights(self, weight: float):
        if not hasattr(self, "frames") or self.frames is None:
            error_exit(f'Cannot find frames attribute for set {self.set_no}')
        for frame in self.frames:
            frame.weight = weight
        self._update_frames()

    #Just becuase its used in set_weights, but defined in the datatype subdataclasses
    def _update_frames(self):
        raise NotImplementedError(
            f'_update_frames not available in {self.set_type}. (Requires dataclass specific to datatype)')

    @staticmethod
    def _find_line(name: str, lines: list[str]) -> int:
        idx = next((i for i, line in enumerate(lines) if name in line), None)
        if idx is None:
            error_exit(f'Error: No {name} found in this set')
        return idx

    @staticmethod
    def _get_subclass(set_type: str) -> Type["ObsSet"]:
        if set_type.lower() == 'delay-doppler':
            return ObsDelayDoppler
        if set_type.lower() == 'doppler':
            return ObsDoppler
        if set_type.lower() == 'lightcurve':
            return ObsLightCurve
        return ObsSet


#===FRAMES INFO===
@dataclass
class ObsLightCurveFrame:
    fname: str
    calfact_freeze: str
    calfact: float
    weight: float
    
    def to_line(self) -> str:
        return (
            f'{self.fname: >25} '
            f'{self.calfact_freeze} {self.calfact:.6e} '
            f'{self.weight:6e} '
            '{name, calfact, weight}\n')

@dataclass
class ObsDopplerFrame(ObsLightCurveFrame):
    date: Time
    sdev: float
    looks: float
    mask: int
    
    def to_line(self) -> str:
        date_str = time_astropy2shape(self.date)
        return (
            f'{self.fname: >25} {date_str} '
            f'{self.sdev:.6e} '
            f'{self.calfact_freeze} {self.calfact:.6e} '
            f'{self.looks: >7} '
            f'{self.weight:6e} '
            f'{self.mask: >4d}\n')
    
@dataclass
class ObsDelayDopplerFrame(ObsDopplerFrame):
    com_del_row: float
    
    def to_line(self) -> str:
        date_str = time_astropy2shape(self.date)
        return (
            f'{self.fname: >25} {date_str} '
            f'{self.sdev:.6e} '
            f'{self.calfact_freeze} {self.calfact:.6e} '
            f'{self.looks: >7} '
            f'{self.com_del_row: >13.6f} '
            f'{self.weight:6e} '
            f'{self.mask: >4d}\n')


#===DOPPLER (CW)===
@dataclass
class ObsDoppler(ObsSet):
    dop_info: list[float] = None
    no_frames: int = None
    frames: list[ObsDopplerFrame] = None
        
    def __post_init__(self):
        self._parse()
        
    def _parse(self):
        res_idx = self._find_line('{dop: ',self.set_lines)
        self.dop_info = [float(x) for x in self.set_lines[res_idx].split()[:3]]
        
        frame_idx = self._find_line('{number of frames}', self.set_lines)
        self.no_frames = int(self.set_lines[frame_idx].split()[0])
        frame_lines = self.set_lines[frame_idx+2 : frame_idx+2 + self.no_frames]
        
        self.frames = [
            ObsDopplerFrame(
                fname=parts[0],
                date=time_shape2astropy(' '.join(parts[1:7])),
                sdev=float(parts[7]),
                calfact_freeze=parts[8],
                calfact=float(parts[9]),
                looks=float(parts[10]),
                weight=float(parts[11]),
                mask=int(parts[12])
            )
            for parts in (line.split() for line in frame_lines)
        ]
    
    def _update_frames(self):
        if not self.frames:
            error_exit(f'Cannot find frames when trying to update lines in set {self.set_no}')
            return

        frame_idx = self._find_line('{number of frames}', self.set_lines)
        start = frame_idx + 2

        for i, frame in enumerate(self.frames):
            self.set_lines[start + i] = frame.to_line()
            
        
#===DELAY DOPPLER===
@dataclass
class ObsDelayDoppler(ObsSet):
    del_info: list[Union[float,str]] = None
    dop_info: list[float] = None
    no_frames: int = None
    frames: list[ObsDopplerFrame] = None
        
    def __post_init__(self):
        self._parse()
        
    def _parse(self):
        del_idx = self._find_line('{delay: ',self.set_lines)
        parts = self.set_lines[del_idx].split()
        self.del_info = [float(parts[0]),float(parts[1]),float(parts[2]),float(parts[3]),parts[4]]      
        dop_idx = self._find_line('{dop: ',self.set_lines)    
        self.dop_info = [float(x) for x in self.set_lines[dop_idx].split()[:5]]
        
        frame_idx = self._find_line('{number of frames}', self.set_lines)
        self.no_frames = int(self.set_lines[frame_idx].split()[0])
        frame_lines = self.set_lines[frame_idx+2 : frame_idx+2 + self.no_frames]
        
        self.frames = [
            ObsDelayDopplerFrame(
                fname=parts[0],
                date=time_shape2astropy(' '.join(parts[1:7])),
                sdev=float(parts[7]),
                calfact_freeze=parts[8],
                calfact=float(parts[9]),
                looks=float(parts[10]),
                com_del_row=float(parts[11]),
                weight=float(parts[12]),
                mask=int(parts[13])
            )
            for parts in (line.split() for line in frame_lines)
        ]
        
    def _update_frames(self):
        if not self.frames:
            error_exit(f'Cannot find frames when trying to update lines in set {self.set_no}')
            return

        frame_idx = self._find_line('{number of frames}', self.set_lines)
        start = frame_idx + 2

        for i, frame in enumerate(self.frames):
            self.set_lines[start + i] = frame.to_line()

            
            

#===LIGHT CURVE===
@dataclass
class ObsLightCurve(ObsSet):
    no_points: int = None
    frames: list[ObsDopplerFrame] = None
        
    def __post_init__(self):
        self._parse()
        
    def _parse(self):

        points_idx = self._find_line('{number of samples', self.set_lines)
        self.no_points = int(self.set_lines[points_idx].split()[0])
        
        parts = self.set_lines[points_idx+1].split()
        frame = ObsLightCurveFrame(
            fname=parts[0],
            calfact_freeze=parts[1],
            calfact=float(parts[2]),
            weight=float(parts[3]),
        )
        self.frames = [frame] #Store in list to match other datasets, but only ever length 1
        
    def _update_frames(self):
        if not self.frames:
            error_exit(f'Cannot find frames when trying to update lines in set {self.set_no}')
            return

        points_idx = self._find_line('{number of samples', self.set_lines)

        self.set_lines[points_idx+1] = self.frames[0].to_line()
            
        